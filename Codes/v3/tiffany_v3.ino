...
struct Hexapod {
  ...
  int andarCircularReacao(int k, float angle, int3 angles){
    ...
    floatxyz xyzEsqMRot = this->EsqM.mapeiaTrajetoriaCircular(this->EsqM.xyz_ini, xyzEsqM, this->EsqM.P3[0] - this->EsqM.xyz_ini.x, angle_max, OmbEsqMSig);
    floatxyz xyzDirTRot = this->DirT.mapeiaTrajetoriaCircular(this->DirT.xyz_ini, xyzDirT, this->DirT.P3[0] - this->DirT.xyz_ini.x, -angle_max, OmbDirTSig);
    // Juncao da trajetoria linear e circular
    xyzEsqF = (xyzEsqF*(v_mult) + xyzEsqFRot*(w_mult))/(v_mult+w_mult);
    xyzDirM = (xyzDirM*(v_mult) + xyzDirMRot*(w_mult))/(v_mult+w_mult);
    xyzEsqT = (xyzEsqT*(v_mult) + xyzEsqTRot*(w_mult))/(v_mult+w_mult);
    xyzDirF = (xyzDirF*(v_mult) + xyzDirFRot*(w_mult))/(v_mult+w_mult);
    ...
    int3 anglesRot = angles*(-1);
    // Aplicar a translacao do ombro corrigido pro centro do corpo
    floatxyz pontoEsqF = xyzEsqF*signalsEsq + OmbEsqF;
    floatxyz pontoDirM = xyzDirM*signalsDir + OmbDirM;
    floatxyz pontoEsqT = xyzEsqT*signalsEsq + OmbEsqT;
    floatxyz pontoDirF = xyzDirF*signalsDir + OmbDirF;
    ...
    floatxyz xyzEsqTReact = (xyzEsqTrot - OmbEsqT)*signalsEsq;
    floatxyz xyzDirFReact = (xyzDirFrot - OmbDirF)*signalsDir;
    floatxyz xyzEsqMReact = (xyzEsqMrot - OmbEsqM)*signalsEsq;
    floatxyz xyzDirTReact = (xyzDirTrot - OmbDirT)*signalsDir;
    
    // De pontos para angulos e movimento
    int3 anglesEsqF = this->EsqF.cinematicaInversa(xyzEsqFReact);
    int3 anglesDirM = this->DirM.cinematicaInversa(xyzDirMReact);
    int3 anglesEsqT = this->EsqT.cinematicaInversa(xyzEsqTReact);
    int3 anglesDirF = this->DirF.cinematicaInversa(xyzDirFReact);
    int3 anglesEsqM = this->EsqM.cinematicaInversa(xyzEsqMReact);
    int3 anglesDirT = this->DirT.cinematicaInversa(xyzDirTReact);
    this->EsqF.moverPata(anglesEsqF.ombro, anglesEsqF.femur, anglesEsqF.tibia);
    this->DirM.moverPata(anglesDirM.ombro, anglesDirM.femur, anglesDirM.tibia);
    this->EsqT.moverPata(anglesEsqT.ombro, anglesEsqT.femur, anglesEsqT.tibia);
    this->DirF.moverPata(anglesDirF.ombro, anglesDirF.femur, anglesDirF.tibia);
...
void TaskHexapod(void *pvParameters) {
  ...
  for (;;) {
    ...
    else if(estado == 3){ // Andar omnidirecional ou rotacional respondendo ou nao a inclinacao
...
void TaskComunicacao(void *pvParameters) {
  for (;;) {
    ...
    if (estado != 11){
      ...
      if (GamePad.isSquarePressed()){       // Modo reacao a inclinacao
        mode = 2;
      }
